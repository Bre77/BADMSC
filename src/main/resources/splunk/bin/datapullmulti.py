...
class Input(Script):
    ...
    def stream_events(self, inputs, ew):
        ...
        checkpointfile = os.path.join(
            self._input_definition.metadata["checkpoint_dir"], index
        )
        ...
        # Ensure Checkpoint is good
        try:
            open(checkpointfile, "r").read()
        except:
            open(checkpointfile, "w").write(str(start))
        ...
        with requests.Session() as s:
            ...
            while True:
                ...
                ew.log(
                    EventWriter.INFO,
                    f"status=search name={index} instance={name} earliest={earliest} latest={latest} start={start} end={end}",
                )
                with s.post(
                    url,
                    stream=True,
                    data={
                        "search": f"search index={index} | reverse",

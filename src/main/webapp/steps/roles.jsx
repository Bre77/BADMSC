...
export default ({ step, config }) => {
    ...
    return Object.entries(src.data)
      ...
      .map(([name, srcContent]) => {
        ...
        const others = FIELDS.map((field) => [field, srcContent[field], dstContent[field]]).filter(
          ([field, src, dst]) =>
            src && src !== dst && (field !== "defaultApp" || allowedApps.includes(src) || console.info(`Ignoring default app '${src}' on role '${name}'`))
        );
        ...
        return [
          ...
          dedup([...srcContent.srchIndexesDisallowed, ...dstContent.srchIndexesDisallowed])
            ...
            .sort()
            .map((index) => [index, SYMBOLS[srcContent.srchIndexesDisallowed.includes(index) + 2 * dstContent.srchIndexesDisallowed.includes(index)]]),
          others,
        ];
      });
  ...
  return (
    <div>
      <P>
        KNOWN ISSUES: If a inherited role doesnt exist, you will need to migrate it first. If a capability still has a plus sign after migration, its inherited.
      </P>
      <Heading level={2}>Step {step}.1 - Create Roles</Heading>
      <P>
        If a roles dont exist, then we cannot set it as inherited. So we can create all new roles first to avoid this issue. If this fails, you may not have
        permission to create certain roles in Splunk Cloud.
      </P>
      ...
      <P>
        Items marked with a plus (+) only exist on the source, while items marked with a tilde (~) only exist in Splunk Cloud. Nothing will be remove from
        Splunk Cloud to avoid breaking functionality, so this section will merge the two together.
      </P>

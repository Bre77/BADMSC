                              ...
                              .then((data) =>
                                  data.total
                                      ? {
                                            ...
                                            version: data.results[0].releases.find((release) => release.product_compatibility.includes("Splunk Cloud"))?.title,
        ...
        <div>
            ...
            {token ? (
                ...
                <>
                    <ControlGroup label="Username">
                        <Text type="username" autocomplete="username" onChange={handleUsername} value={username} required></Text>
                    </ControlGroup>
                    <ControlGroup label="Password">
                        <Text type="password" autocomplete="current-password" onChange={handlePassword} value={password} required></Text>
                    </ControlGroup>
                    ...
                    <Table.Body>
                        {apps.splunkbase.map(({ name, local, splunkbase }) => (
                            <Table.Row key={name}>
                                ...
                                <Table.Cell>
                                    <InstallSplunkbase config={config} splunkbase={splunkbase} token={token.splunkbaseToken} />
                                </Table.Cell>
                            </Table.Row>
                        ))}
                    </Table.Body>
            ...
            <List>
                {apps.unsupported.map(({ name, local, splunkbase }) => (
                    <List.Item key={name}>
                        <Link to={`https://splunkbase.splunk.com/app/${splunkbase.uid}`} openInNewContext>
                            {name}
                        </Link>
                    </List.Item>
                ))}
                            ...
                            if (data.summary.error > 0 || data.summary.failure > 0 || data.summary.manual_check > 0) {
                                ...
                                console.warn(
                                    ...
                                    data.reports.flatMap((a) =>
                                        a.groups.flatMap((b) =>
                                            b.checks.filter((c) => ["failure", "error", "manual_check"].includes(c.result)).flatMap((c) => c.messages)
                                        )
                                    )
                                );
